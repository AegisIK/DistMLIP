...
class TensorNet_Dist(TensorNet):
    def enable_distributed_mode(self, gpus):
        ...
        self.bond_expansion_dist = [deepcopy(self.bond_expansion).to(gpu_index).eval().to(matgl.dist_float_th) for gpu_index in gpus]
        self.tensor_embedding_dist = [deepcopy(self.tensor_embedding).to(gpu_index).eval().to(matgl.dist_float_th) for gpu_index in gpus]
        self.layers_dist = [[deepcopy(layer).to(gpu_index).eval().to(matgl.dist_float_th) for layer in self.layers] for gpu_index in gpus]
    ...
    def potential_forward_dist(
        ...
        node_types = torch.tensor([self.element_to_index[elem] for elem in atoms.get_chemical_symbols()], dtype=matgl.int_th, device=self.gpus[0])
        ...
        for partition_i, gpu_index in enumerate(self.gpus):
            ...
            X, _, _ = self.tensor_embedding_dist[partition_i](atom_graphs[partition_i], state_attr)
            X_feats.append(X)
        
        return node_types, big_graph_positions, strain, self.dist_forward(atom_graphs, X_feats, dist_info)
    ...
    def dist_forward(self, atom_graphs, X_feats, dist_info):
        ...
        for layer_i in range(len(self.layers)):
            for partition_i, gpu_index in enumerate(self.gpus):
                X_feats[partition_i] = self.layers_dist[partition_i][layer_i](atom_graphs[partition_i], X_feats[partition_i])
        ...
        if self.is_intensive:
            raise NotImplementedError("self.is_intensive = True is not yet supported by distributed inference")
