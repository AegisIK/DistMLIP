...
class CHGNet_Dist(CHGNet):
    ...
    def potential_forward_dist(
        ...
        for partition_i, gpu_index in enumerate(self.gpus):
            ...
            bond_expansion = self.bond_expansion_dist[partition_i](
                atom_graphs[partition_i].edata["bond_dist"]
            )
            smooth_cutoff = polynomial_cutoff(
                bond_expansion, self.cutoff, self.cutoff_exponent
            )
            atom_graphs[partition_i].edata["bond_expansion"] = (
                smooth_cutoff * bond_expansion
            )
        ...
        if self.use_bond_graph:
            ...
            for partition_i, gpu_index in enumerate(self.gpus):
                ...
                this_bond_graph = dgl.graph(
                    (
                        dist_info.line_src_nodes[partition_i],
                        dist_info.line_dst_nodes[partition_i],
                    ),
                    num_nodes=num_nodes,
                ).to(gpu_index)

                # Send bond_dist data from edges in atom_graph to nodes in this_bond_graph
                this_bond_graph.ndata["bond_dist"] = dist_info.edge_to_bond(
                    atom_graphs[partition_i].edata["bond_dist"],
                    partition_i,
                    device=gpu_index,
                )
                ...
                this_bond_graph.ndata["bond_vec"] = dist_info.edge_to_bond(
                    atom_graphs[partition_i].edata["bond_vec"],
                    partition_i,
                    device=gpu_index,
                )
            ...
            dist_info.bond_transfer(
                [bond_graph.ndata["bond_dist"] for bond_graph in bond_graphs]
            )
            dist_info.bond_transfer(
                [bond_graph.ndata["bond_vec"] for bond_graph in bond_graphs]
            )
            ...
            for partition_i, gpu_index in enumerate(self.gpus):
                ...
                threebody_bond_expansion = self.threebody_bond_expansion_dist[
                    partition_i
                ](this_bond_graph.ndata["bond_dist"])
                smooth_cutoff = polynomial_cutoff(
                ...
                this_bond_graph.ndata["bond_expansion"] = (
                    smooth_cutoff * threebody_bond_expansion
                )

                # Set up src_bond_sign and bond_index
                this_bond_graph.edata["center_atom_index"] = torch.tensor(
                    dist_info.local_center_atom_indices_list[partition_i],
                    device=gpu_index,
                    dtype=torch.int64,
                )
                this_bond_graph.ndata["src_bond_sign"] = -1 * torch.ones(
                    num_nodes, 1, device=gpu_index
                )

                this_bond_graph.apply_edges(compute_theta)
                this_bond_graph.edata["angle_expansion"] = self.angle_expansion_dist[
                    partition_i
                ](this_bond_graph.edata["theta"])
        ...
        return (
            ...
            strain,
            self.dist_forward(
                atom_graphs=atom_graphs, bond_graphs=bond_graphs, dist_info=dist_info
            ),
        )
    ...
    def dist_forward(
        ...
        if state_features is not None:
            raise NotImplementedError(
                "State features not implemented for distributed computation."
            )
        ...
        for partition_i in range(num_partitions):
            ...
            atom_features.append(
                self.atom_embedding_dist[partition_i](
                    curr_atom_graph.ndata["node_type"]
                )
            )
            atom_edge_features.append(
                self.bond_embedding_dist[partition_i](
                    curr_atom_graph.edata["bond_expansion"]
                )
            )
            ...
            if self.use_bond_graph:
                ...
                angle_features.append(
                    self.angle_embedding_dist[partition_i](
                        curr_bond_graph.edata["angle_expansion"]
                    )
                )
        ...
        if self.use_bond_graph:
            ...
            for partition_i, gpu_index in enumerate(self.gpus):
                ...
                this_bond_node_feature = dist_info.edge_to_bond(
                    atom_edge_features[partition_i], partition_i, device=gpu_index
                )
        ...
        if self.atom_bond_weights:
            atom_bond_weights = [
                self.atom_bond_weights_dist[partition_i](
                    atom_graphs[partition_i].edata["bond_expansion"]
                )
        ...
        if self.bond_bond_weights:
            bond_bond_weights = [
                self.bond_bond_weights_dist[partition_i](
                    atom_graphs[partition_i].edata["bond_expansion"]
                )
                for partition_i in range(len(self.gpus))
            ]
        ...
        if self.threebody_bond_weights:
            threebody_bond_weights = [
                self.threebody_bond_weights_dist[partition_i](
                    bond_graphs[partition_i].ndata["bond_expansion"]
                )
                for partition_i in range(len(self.gpus))
            ]
        ...
        if self.use_bond_graph:
            ...
            for layer_i in range(self.n_blocks - 1):
                for partition_i, gpu_index in enumerate(self.gpus):
                    atom_features[partition_i], atom_edge_features[partition_i], _ = (
                        self.atom_graph_layers_dist[
                            partition_i
                        ][
                            layer_i
                        ](
                            atom_graphs[partition_i],
                            atom_features[partition_i],
                            atom_edge_features[partition_i],
                            None,
                            atom_bond_weights[partition_i],
                            bond_bond_weights[partition_i],
                        )
                    )

                    # Update bond_node_features with newly computed atom_edge_features
                    dist_info.edge_to_bond(
                        atom_edge_features,
                        partition_i,
                        inplace=True,
                        bond_node_features=bond_node_features,
                    )
                ...
                for partition_i, gpu_index in enumerate(self.gpus):
                    ...
                    bond_node_features[partition_i], angle_features[partition_i] = (
                        self.bond_graph_layers_dist[
                            partition_i
                        ][
                            layer_i
                        ](
                            bond_graphs[partition_i],
                            atom_features[partition_i],
                            bond_node_features[partition_i],
                            angle_features[partition_i],
                            threebody_bond_weights[partition_i],
                            compute_dist=True,
                            convolution_type="node",
                        )
                    )

                    # Update the atom_edge_features with the new output
                    dist_info.bond_to_edge(
                        bond_node_features, atom_edge_features, partition_i
                    )
        ...
        site_properties_list = [
            self.sitewise_readout_dist[partition_i](atom_features[partition_i])
            for partition_i in range(num_partitions)
        ]
        site_properties_aggregate = dist_info.aggregate(
            site_properties_list, self.gpus[0]
        )
        ...
        if self.readout_field == "atom_feat":
            ...
            structure_properties_aggregate = readout_nodes(
                big_graph_placeholder, "atom_features", op=self.readout_operation
            )
            ...
            raise NotImplementedError(
                "bond_feat readout not yet supported. Let me (Kevin/AegisIK) know if you need this."
            )
        elif self.readout_field == "angle_feat":
            raise NotImplementedError(
                "angle_feat readout not yet supported. Let me (Kevin/AegisIK) know if you need this."
            )
        else:
            raise Exception(
                f"Unknown self.readout_field value of: {self.readout_field}"
            )
    ...
    def enable_distributed_mode(self, gpus):
        ...
        self.bond_expansion_dist = [
            deepcopy(self.bond_expansion).to(gpu_index).eval() for gpu_index in gpus
        ]

        self.threebody_bond_expansion_dist = (
            [
                deepcopy(self.threebody_bond_expansion).to(gpu_index).eval()
                for gpu_index in gpus
            ]
            if self.use_bond_graph
            else None
        )
        self.angle_expansion_dist = (
            [deepcopy(self.angle_expansion).to(gpu_index).eval() for gpu_index in gpus]
            if self.use_bond_graph
            else None
        )

        if self.atom_bond_weights:
            self.atom_bond_weights_dist = [
                deepcopy(self.atom_bond_weights).to(gpu_index).eval()
                for gpu_index in gpus
            ]

        if self.bond_bond_weights:
            self.bond_bond_weights_dist = [
                deepcopy(self.bond_bond_weights).to(gpu_index).eval()
                for gpu_index in gpus
            ]

        if self.threebody_bond_weights:
            self.threebody_bond_weights_dist = [
                deepcopy(self.threebody_bond_weights).to(gpu_index).eval()
                for gpu_index in gpus
            ]
        ...
        self.atom_embedding_dist = [
            deepcopy(self.atom_embedding).to(gpu_index).eval() for gpu_index in gpus
        ]
        self.bond_embedding_dist = [
            deepcopy(self.bond_embedding).to(gpu_index).eval() for gpu_index in gpus
        ]
        self.angle_embedding_dist = (
            [deepcopy(self.angle_embedding).to(gpu_index).eval() for gpu_index in gpus]
            if self.use_bond_graph
            else None
        )

        if self.state_embedding is not None:
            self.state_embedding_dist = [
                deepcopy(self.state_embedding).to(gpu_index).eval()
                for gpu_index in gpus
            ]

        self.sitewise_readout_dist = [
            deepcopy(self.sitewise_readout).to(gpu_index).eval() for gpu_index in gpus
        ]

        self.final_layer_dist = [
            deepcopy(self.final_layer).to(gpu_index).eval() for gpu_index in gpus
        ]
