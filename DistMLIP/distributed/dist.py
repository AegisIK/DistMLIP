...
class Distributed:
    ...
    def __init__(self, src_nodes, dst_nodes, markers,
                local_coords, global_ids, py_index_1,
                py_index_2, py_offsets, py_distances,
                line_src_nodes, line_dst_nodes, within_r_indices,
                line_markers, num_UDEs_per_partition, bond_mapping_DE_list,
                bond_mapping_UDE_list, L2G_DE_mapping_list, G2L_DE_mapping_list,
                local_center_atom_indices_list, use_bond_graph,
        ...
        self.L2G_DE_mapping_list = L2G_DE_mapping_list # index i is the global id associated with local id i
    ...
    @classmethod
    def create_distributed(cls, cart_coords, frac_coords, lattice_matrix, num_partitions, pbc, cutoff, three_body_cutoff, tol, use_bond_graph=False, num_threads=1):
        ...
        if use_bond_graph:
            assert len(line_markers) == 0 or len(line_markers) == num_partitions, f"Length of line_markers is {len(line_markers)} which is not {num_partitions} or 0."
            line_markers_tmp = []
            for i in range(len(line_markers)):
                line_markers_tmp.append(np.append(line_markers[i], num_UDEs_per_partition[i]))
            line_markers = line_markers_tmp
        else:
            line_markers = None

        return cls(src_nodes, dst_nodes, markers,
                local_coords, global_ids, py_index_1,
                py_index_2, py_offsets, py_distances,
                line_src_nodes, line_dst_nodes, within_r_indices,
                line_markers, num_UDEs_per_partition, bond_mapping_DE_list,
                bond_mapping_UDE_list, L2G_DE_mapping_list, G2L_DE_mapping_list,
                local_center_atom_indices_list, use_bond_graph, len(cart_coords))
    ...
    def aggregate(self, features_to_aggregate, device="cpu", aggregate_dim=None):
        ...
        combined_feats = torch.empty((aggregate_dim, ) + tuple(features_to_aggregate[0].shape[1:]), device=device, dtype=features_to_aggregate[0].dtype)
    ...
    def transfer_nodes(self, features, markers):
        ...
        for curr in range(self.num_partitions):
            for to in range(self.num_partitions):
                ...
                if from_start != from_end: # Only attempt to transfer if there is stuff to transfer
    ...
    def aggregate_atom_edge(self, atom_edge_features, gpu_to_aggregate_to="cpu"):
        ...
        aggregated_feats = torch.empty((self.total_num_edges, ) + tuple(atom_edge_features[0].shape[1:]), device=gpu_to_aggregate_to, dtype=atom_edge_features[0].dtype)
    ...
    def aggregate_bond_node(self, bond_node_features, gpu_to_aggregate_to):
        ...
        placeholder_dims = [tuple([len(self.src_nodes[i])] + bond_node_features[0].shape[1:]) for i in range(self.num_partitions)]
        atom_edge_features_placeholders = [torch.zeros(placeholder_dims, device=gpu_to_aggregate_to, dtype=bond_node_features[0].dtype)]

        for i in range(self.num_partitions):
            atom_edge_features_placeholders[i][self.bond_mapping_DE_list[i]] = bond_node_features[i][self.bond_mapping_UDE_list[i]]

        return self.aggregate_atom_edge(atom_edge_features_placeholders, gpu_to_aggregate_to=gpu_to_aggregate_to)
    ...
    def global_to_local_nodes(self, global_node_features, partition, device="cpu", inplace=False):
        if inplace:
    ...
    def global_to_local_edges(self, global_edge_features, partition, device="cpu", inplace=False):
        if inplace:
            return global_edge_features[self.L2G_DE_mapping_list[partition]].to(device)
        return global_edge_features.clone()[self.L2G_DE_mapping_list[partition]].to(device)

    def edge_to_bond(self, edge_features, partition, device="cpu", inplace=False, bond_node_features=None):
        """
        Tranfers edge features in atom graph to node features in bond graph 
        ...
        """
        if inplace:
            assert bond_node_features, "bond_node_features cannot be None if inplace is True"
            bond_node_features[partition][self.bond_mapping_UDE_list[partition]] = edge_features[partition][self.bond_mapping_DE_list[partition]]
            return
        
        bond_features = torch.zeros((self.num_bonds(partition), ) + tuple(edge_features.shape[1:]), device=device)
    ...
    def bond_to_edge(self, bond_node_features, atom_edge_features, partition):
        ...
        """
        atom_edge_features[partition][self.bond_mapping_DE_list[partition]] = bond_node_features[partition][self.bond_mapping_UDE_list[partition]]
